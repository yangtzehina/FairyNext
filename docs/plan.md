...
- **M1-23 孤岛②③④**（~1200 行）：AddIsland/IIslandContent（含 StillAnimating）、visual 并入下行、②自定义材质（clip include/scissor 降级）、③外部原生（SortingGroup 对齐 run 序、Spine kind）、④stencil 括号。验收项（2026-08 审计遗留）：① `AnyIslandAnimating` 是零脏帧短路的第三前提，M1-14 阶段恒 false 无门——本包必须补「活孤岛自报 StillAnimating ⇒ stats.Dirty 为真、presents 照涨」的正例与「自报静止 ⇒ 短路恢复」的收据断言，否则第三前提仍是无人执法的纸面条款。② **孤岛表进规范形**——`CanonicalStream` 的首用序重编号只盖槽表与 clip 表，孤岛挂载记录不在规范化范围内；本包落地 AddIsland 时必须把孤岛表纳入同一套「分配序非身份」的重编号（否则增量门与 Trace 哈希对孤岛记录要么漏比要么把分配序漂移误报为差异）。M1-15 留缝：`UnityVertexStreamBackend` 对孤岛只记账不产原生对象（Create/Sync/Destroy 句柄面已备）——本包在 Unity 侧补 SortingGroup 对齐 run 序与 kind 分派。 M1-21 转登记：**`Ch.DownLayer` 的首个产品写者候选之一**——「孤岛 visual 并入下行」若落成「整棵子树翻层」，本包即是首个 Mark 者，届时必须补「DownLayer 级联落到命中/绘制序」的行为用例并改掉 EventTests 里那道登记门（`DownLayerHasNoProductWriterYet`）；若本包判定孤岛 visual 仍走 DownColor/DownVisible，则把该登记原样转给 M2-12。 **已交付**：`src/Core/Rendering/IslandContent.cs`（内容侧接口面：`IIslandContent`（`AcceptsClipInclude`/`StillAnimating`/`OnAttach`/`OnSync`/`MarkDirty`/`OnDetach`）、`IslandContext`（槽矩阵/裁剪矩形/**深度区间 [ZMin,ZMax)**/分频位/回调句柄）、`IslandMount`（**内容 → 运行时**的自报脏，落成节点的 `Ch.Content` 上行标记）、三个封闭枚举 `IslandNativeKind`（③的具名 kind：Spine/DragonBones/Custom，**核心零 Spine 依赖**）/`IslandClipMode`（None/ShaderInclude/Scissor）/`IslandBracket`（None/Enter/Exit））+ `IslandTable.cs`（登记表 + **P7 收尾同步**：算挂载事实 → 变了先通知内容重画 → `OnSync` → 问自报 → 连 visual/clip/序一起下发后端；离开检测按「本帧没在本流里出现过」，故**一表一面板**）。改动：`RenderStream`（**孤岛占一个 run 序是字面的**——run 与孤岛共用一个序游标 `_orderCursor`，孤岛的 sortingOrder 因此**严格介于**前后两个 run 之间，SortingGroup 对齐由此成为分配事实而非纪律；新增 `SetIslandVisual`（位等切断）/`MarkIslandDirty`（帧锁存，`BuildStats` 读完即清）/`Island(i)`；`IslandRecord` 增 Node/NativeKind/Bracket/StencilDepth/ClipMode/PaintOrderIndex）、`Extract`（孤岛绑节点与内容、②的 clip include/scissor 二分、**④stencil 括号按 paintOrder 子树区间 LIFO 闭合**、深度预算 8 溢出走新阶梯 `StencilDepthOverflow`）、`StreamDrain`（孤岛反查表 + Color/Visible 的孤岛臂（可见性跃迁照旧升级 Structure）+ **Content 通道的孤岛臂不升级 Structure**——把每次 Spine 帧推进判成结构变化是把最常见的外部动效变成最贵的路径）、`RenderPipeline`（`AddIsland`/`RemoveIsland` 接缝、下行落点含孤岛、P7 收尾同步；**流的首次 Attach 从 P8 挪到 P7 收尾**——否则首帧孤岛拿不到后端句柄，那一帧的 SyncIsland 整个丢掉）、`StreamStructureGate`（**孤岛进门**，M1-14b 登记结账）、`CanonicalStream`（**孤岛表进规范形**，FormatVersion 1→2；剔 Node/Handle/StillAnimating/DebugName，槽与 clip 走同一套首用序重编号）、`StreamSnapshot`/`IncrementalGate`（神谕腿共用同一张孤岛表）、`MockBackend`（后端侧孤岛表按**建岛序**进快照）、Unity 后端（**每岛一个 GameObject + SortingGroup**，序与段同出一本 run 序账；槽矩阵/可见性每帧下发；`IslandRoot(handle)` 给宿主挂外部渲染器；材质注入与 stencil 状态机随滤镜 RT 同批 M2-12）。**验收项①结账**：`AnimatingIslandBlocksTheZeroDirtyShortcut`（自报在动 ⇒ 提交零调用但 `stats.Dirty` 为真、presents 照涨）+ `StillIslandRestoresTheShortcut`（停了 ⇒ ticks 涨 presents 不涨、七条队列全零）。**验收项②结账**：孤岛表进规范形，逐字段用例（九个语义字段各自改一个即字节变；DebugName 与节点句柄有意缺席）+ 分配序重编号用例 + 增量门红灯用例（孤岛 visual 漂移 ⇒ 定位到 `Islands[i].visual ⇐ Color|Visible`）。**M1-21 登记转手**：孤岛跟随的是 α/置灰/隐藏三元（DownColor/DownVisible），它的「层」是自己那一个 run 序（Ch.Structure）——**没有产品写者**，`DownLayerHasNoProductWriterYet` 原样转 M2-12，本包只补了 DownLayer 的**消费者**覆盖（下钻落到孤岛）。用例 22 条（`tests/FairyNext.Tests/IslandTests.cs`）；杀变异 22 例注入 21 例被杀（存活 1 例：mock 后端孤岛快照的「按建岛序排」是防御性代码，当前分配器整批销毁+整批重建 + first-fit 恒升序，结构上造不出倒序语料）。
