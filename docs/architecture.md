...
- `AddIsland(NodeId, IslandKind, IIslandContent)`；`IIslandContent { OnAttach(ctx: 槽/clip 矩形/深度区间), MarkDirty(), StillAnimating }`（M1-23 落地时**扩了三个成员**，见平面三实现期补充：`OnSync(ctx)` 是「每帧下发」这条契约的调用点、`OnDetach()` 是句柄纪律的对手方、`AcceptsClipInclude` 是②号条目 include/scissor 二分的声明面；内容 → 运行时的自报脏走 `ctx.Mount.MarkDirty()`，与运行时 → 内容的 `MarkDirty()` 是两个方向、两个类型）。
...
>
> **顶点流后端的 Unity 落地形态（M1-15）。** 机制 12 的顶点流半边在 Unity 侧的选型全部落定：**每段一个 MeshRenderer**，80B 实例四角展开成 60B/顶点（position + uv + `uint4`(color/route/flags/aux) + extra + quad 内归一化角），整数字段一律 `VertexAttributeFormat.UInt32`；**y 翻转唯一点**兑现机制⑪原文——每条流根 GameObject 的 `localScale=(1,-1,1)`，顶点/槽矩阵/clip 条目/shader 全程留在流的 y-down 空间，绕序反向由 `Cull Off` 消化；**fence 回收**消费 `Abi.GpuFenceDepth`——销毁先 `SetActive(false)`（同帧 Destroy 不生效，僵尸段污染像素探针是 fork 实测）再入坟场，BeginFrame 时入队满深度帧者才真 Destroy，pending 中句柄收到调用 = use-after-free 进 Violations；槽与 clip 走 uniform 数组（2 vec4/槽 + **逆矩阵数组**——clip 条目的 rect 在它自己绑的槽帧里，像素经流根空间 × 逆矩阵换帧求值；3 vec4/条）；径向填充按 `RadialFill.PackExtra` 的求值形态（中心 + 起角 + 有符号扫角，turns）在 shader 一条判据完成；shader 的 ABI 宏取自 `unity/Assets/FairyNext/Shaders/abi.g.hlsl`——AbiGen 的**第四份生成物**（与 shaders/ 那份逐字节同源，Unity include 解析不出工程目录，副本纪律同 AbiMock.g.cs）。明文边界：`SupportsOffscreen=false` 至 M2-12；孤岛只记账不产原生对象至 M1-23（**已交付**，见下条）；`MeshRenderer.sortingOrder` 是 int16 语义，paintOrder×16 超限夹紧并计违约（WebGL2 后端按数组序绘制无此上限，M3-03）。
>
> **孤岛协议的落地形态（M1-23 实现期补充）。** 机制 9 的清单（②材质／③外部原生／④stencil）与三条随流契约在这一包全部拿到可执行形态，六条明文：
> ① **「占一个 run 序」是字面的**——run 与孤岛共用流里的**一个序游标**，`AddIsland` 从中拿走一个下标，后面那个 run 从再下一个起。于是「孤岛的 sortingOrder 严格介于前后两个 run 之间」是**分配事实**而不是纪律；Unity 的 `SortingGroup` 与段的 `sortingOrder` 读的是同一本序账，穿插错乱（fork GoWrapper 时代「Spine 忽前忽后」的病根）在结构上不可能发生。此前的形态是「孤岛只关闭当前 run」，它与后面那个 run 撞号。
> ② **visual 并入下行 = 与叶字面同源的两条路**：α/置灰变化走 `Ch.Color` 原位写孤岛记录（不整编）；**可见性跃迁走整编**——孤岛要进/出流是数量变化，原位把 visual 清成不可见画面也对，但整编产物里根本没有那条记录，逐字节门当场变红。进/出流的判据是 Extract 的 authored 父链传播，**不是 worldVisual**（隐藏子树后代的 worldVisual 合法陈旧，14b CRITICAL 同型）；读 worldVisual 取 α 的合法性也与叶同源：在流里的孤岛按进流条件没有隐藏祖先。
> ③ **两个方向的 MarkDirty 住在两个类型上**：`IIslandContent.MarkDirty()` 是**运行时 → 内容**（「你的挂载参数变了，自己重画」）；`IslandMount.MarkDirty()` 是**内容 → 运行时**（外部内容没有失效通道，自报脏落成节点的 `Ch.Content` 上行标记）。后者在 P7 被消化成一次孤岛同步 + 本帧脏，**不升级 Structure**——把每一次 Spine 帧推进判成结构变化，等于把最常见的外部动效变成最贵的路径。
> ④ **④stencil 是一个节点两条记录**：`Enter` 在子树之前、`Exit` 在子树之后，按 paintOrder 的子树区间闭合、嵌套 LIFO。「进了没出」在 stencil 路径上的症状是整屏被裁掉，与「内容没画」无法区分，所以闭合由 Extract 执法而不是靠调用方自觉。深度预算 8，溢出该孤岛不进流并计 `StencilDepthOverflow`（模板值回绕的症状是「某些元素随机消失」）。
> ⑤ **②的 clip 二分由内容声明**：`AcceptsClipInclude` 为真走 shader include（正路），为假退屏幕轴对齐 scissor 并计一次 `ScissorFallback`；③外部渲染器与④默认走 scissor（它们不吃我们的 shader）。无声降级是被禁的那一种——scissor 在旋转裁剪下没有静默正确解。
> ⑥ **P7 收尾同步的顺序是契约**：先算本帧挂载事实（槽矩阵/clip 矩形/深度区间 [zMin,zMax)/分频位）→ 变了先通知内容重画 → `OnSync` → **最后**问 `StillAnimating` → 连 visual/clip/序一起下发后端。反过来做（先问在不在动、再改参数）会让「参数刚变、内容还没重画」的那一帧被判成静止而跳掉 present。同批把**流的首次 Attach 从 P8 挪到 P7 收尾**：否则首帧 P7 时 `stream.Handle` 还是 None，孤岛拿不到后端句柄，那一帧的 `SyncIsland` 整个丢掉。代价明文：**流里有孤岛时静止帧仍有 O(孤岛数) 的 CPU 同步**——`StillAnimating` 必须每帧问，否则「刚开始动」的那一帧没人发现；孤岛数是个位数，这是「外部内容没有失效通道」的定价。
> Unity 侧：**每岛一个 GameObject + `SortingGroup`**（建在流根之下，继承那一处 y 翻转），槽矩阵与可见性每帧下发，`IslandRoot(handle)` 给宿主挂外部渲染器；材质注入（②）与 stencil 状态机（④）需要 CommandBuffer/专用材质，随滤镜 RT 同批（M2-12）。
...
> **pivotCorrect 落地为「锚点钉住」算子形态（M1-20a 实现期补充）。** 上条 ③ 的保留位就位：置位入口是 `ConstraintGraphBuilder.PinAnchor(dst, axis)`，只出现在 Pin 算子上，把约束方程从「钉边值 `pos = c`」改写成「钉锚点 `pos + pivot × size = c`」——c 在建立/重捕获时取 **authored 锚点**（`apos + pivot × asize`，与 ② 的捕获常量纪律同款）。**修正系数不烘第二份存储**：系数就是 dst 的 authored pivot 分量，pivot 无 resolved 形态、编译后不变，求值现场读列与「烘为常量」逐位同。求值两形态：单独出现解 `pos = c − pivot × size`（无源不进 FanOut，只在布防初解/authored 重捕获时求值）；与写 Size 的算子同组时按拉伸对联立（方程行 `(1, pivot)` × `(0, 1)`，行列式恒非零）——**Seal 校验带伙伴的锚定其伙伴必须写 Size**（与位置算子配对时 pivot 取某些值会使两行线性相关）。编译器的注入判据（pivotAsAnchor 消灭的后半）：**fork 对「锚点节点被尺寸关系拉伸」有两套定点，判据必须分开**（`RelationItem.cs` 391-431 @ oracle 08a2d56）——① 关系源是**组件根**（fork 的 `_target == _owner.parent`，编辑器 `target=""`）：fork 走 `tmp = xMin; SetSize(w, h, ignorePivot:true); xMin = tmp;`，那三行的存在就是为了把 `SetSize` 的保锚点**显式撤销**掉，定点是**原点**；裸 Size 算子的求值恰是这条（`npos = pos`），故**不注**即对齐。② 关系源是**兄弟**：fork 走 `_owner.width = …` ⇒ `SetSize(v, _rawHeight)` ⇒ pivotAsAnchor 分支只 `HandlePositionChanged()`、`_x` 不动，定点是**锚点**；这条要锚定算子与尺寸算子配成联立对才成立。于是判据 = 该轴**恰一个算子 ∧ 它写 Size ∧ 它的源不是局部 0 ∧ 该轴 pivot 非零**。位置已被约束的轴不注——fork 在「同轴既有位置随动又有尺寸随动」时事件序互踩、产物不定形，这里取「显式约束优先」的不动点语义（已知语义差异，像素矩阵 M2-14 按 oracle 已知错豁免口径处理）。**pivot 只有一份来源**：注入判据与求值现场都读 NodeTable 的 pivot 列（组节点也写——不变量 9 的组纯度管的是 scale/rotation/contentRef，且恒等基下 pivot 项在 `LocalMatrix` 自相消），系数不可能分叉。前半（原点换算 `origin = 锚点 − pivot × size`）在编译器建树期烘死，运行时不存在 pivotAsAnchor 概念（机制⑪ 原文兑现）。
...
> **M1-17 实现期落地形态（字形源 + GlyphStore 首版）。** ① **双门的 M1 判据**：落地为「引用计数归零 ∧ 页龄（自最后触碰起的帧距）≥ 阈值」——页账的 idleFrames 语义由帧距实现，引用登记（`AddPageRef`/`ReleasePageRef`，页级、叶生命周期对称）同时是换代 fan-out 的目标集。终态的「内存水位」半边要等真实字节水位存在（M2-09 栅格化落地）才有对象可量，届时水位作为**发起扫描的策略**叠加在同一 `RequestEviction` 之上，双门判据本体不变。② **申请与生效隔半帧 ⇒ 提交时二次验门**：`RequestEviction` 任何时刻只挂起（经 `RequestGlobalInvalidate(GlyphStore)`），P2 窗口提交时按申请时的阈值**重查双门**——期间又被引用/触碰的页原样留店（收据 `GateRejectsAtCommit`）。「换代帧引用叶被全局失效重排」的执法路径是核按钮 `RequestRebuild`（三件套③）：不验门全库退休，退休页登记过的引用叶逐个 `Mark(Ch.Content, GlobalInvalidate)`；常规淘汰过了门 1，fan-out 目标集必然为空。③ **钩子形态**：店经 `AttachInvalidation` 独占 `Invalidation.GlyphStoreFanOut`（M1-14b Attach 同款硬独占，第二家店 throw）；generation 只在窗口内真有页退休时 +1，空窗口不空转版本号。④ **双半区的类型隔离**：度量半区是独立类 `GlyphMetricsTable`（faces + 度量账），不持有任何通往 GlyphStore/页账的引用——「度量永不因页淘汰而变」由结构保证，行为侧另有位等对拍用例。位置账条目不可变，死活派生自所在页（「淘汰整页不是单字形」的结构形态）；`ResidencyState.Pending` 已在词表但 M1-17 无产者（同步落账），异步烘焙随 M2-09 进驻。
...
> **M1-18 实现期落地形态（无状态排版 + 文本管线接线首版）。** ① **`ITextQuadEmitter` 签名偏离定稿**：落地为 `Emit(uint textRef, float width, float height, Span<QuadInstance>, Span<uint> baseColors) → int`，不是定稿的 `Emit(Span<QuadInstance>, in LayoutResult)`。病因两条：`TextLayoutResult` 是指向发射器 arena 的 ref struct，跨接口传出等于给「出不了帧」的类型承诺打洞；且 P7 原位重写臂（Transform 移动、增量门神谕腿的同帧重发射）要求叶可从 `(textRef, w, h)` 任意次重现——排版在 Emit 内部重跑，产物不过界。配套两条纪律入了接口契约：实现帧内纯（同入参逐位同 quad，违者增量门红）、页引用按集合调和（重复发射零副作用）。② **文本叶从第一天可复跑 ⇒ tier-2 例外条款退役**：M1-14 回写给「M1-18 之前的文本叶」保留的 `RestampLeaf` 通道不再需要，文本 Transform 走与 Image 同一条「按新 world 重发射并烘 rect」路径，逐位相等由构造保证；`RestampLeaf` 此后只属于 M2 tween 直写小道。③ **三级惰性第二级 = ProductStamp**：`Ch.Text` 排水（P5 第一条）对脏文本重排取指纹（定位字形流 + 行表 + 尺寸 + 发射可见样式量的 FNV-1a），变才 `Mark(Content, LayoutDerived)`；autoSize 则度量变才 `Mark(Layout)`，由同帧 P5 稍后的度量层接力（排水序 Text→Layout→LayoutStep 已钉）。漏推不可能、多推允许：stamp 混入 (w,h)，尺寸本帧稍后被布局改动时由 `SetResolved` 的派生 Mark 兜底。终态的「度量缓存键 ⊗ generation」分层缓存（上文 measure 免重排）尚未进驻——M1 每次重排，stamp 只省 Content 下游。④ **到货补脏是独立失效因由 `ResidencyDelivered`**，不是 `GlobalInvalidate`：字形像素到货不换代、不走 P2 窗口（老 UV 恒有效），只对订阅叶定向 Mark Content——归因分开，CJK 淡入频次才有账可查。⑤ **M1 明码边界两条**：单纹理页帐——文本叶段键是单一图集纹理，一个叶的全部字形须落同页，异页字形 α=0 静音 + `CrossPageMuted` 收据（宁可不显示，不画错 UV；per-page 纹理绑定随 M2-09 解除）；驻留默认同步（`SyncResidency`，M1 无异步烘焙产者）——关掉即异步形态的行为面（缺字形 α=0 占位 + pending 订阅队列），`DeliverPending` 是烘焙完成回调的 M1 替身。⑥ **P5 度量层的落地形态**：`RegisterMeasured` 声明 auto 轴归属（resolved 槽 + LayoutOwn 位，与 `Arm`/`RegisterLinear` 同形态），`MeasurePass` 为四层第一层、幂等门与差分腿全量复用（度量是 (文本, 样式, availWidth) 的纯函数）；度量节点被约束/流式层改宽自动重入量（防自触发哨兵，一轮收敛），「文本尺寸→约束→回改宽度」回路由既有 ≤3 轮受控窗兜底。
...
> **M1-21 实现期落地形态（命中与派发首版）。** ① **HitMode 的落地是冷侧表不是节点列**：`HitPolicyTable` 按节点下标稀疏挂 `hitMode / opaque / touchChildren` 三件，**不进 `Visual` 位域**——它们是命中策略，唯一消费者是 P0，既不参与 worldVisual 级联也没有渲染侧读者；进位域就得给它们编一个上行通道与一次排水，而没有任何排水器会消费。真需要 gear/timeline 驱动时再进 PropId 空间，那时它自带通道。`opaque` 的默认值由 **typeId 推定**（Image/Text/Shape/Loader 恒兜底，Component/Group/List/Island 默认不兜底）——这正是 fork 两条路径（`DisplayObject.HitTest` 恒返回自己 vs `Container.HitTest_Container` 要 `opaque`）在单树上的合并形态；`HitMode.None` = fork 的 `Flags.TouchDisabled`（整支黑洞），与 `touchable=false` 的区别是它不动 Visual 位、不推子树重算。非 Rect 的模式（PixelTest/Shape/Custom）是**门**：判否即整支不下行（fork `hitArea` 同语义），Rect 不是门（容器框外的孩子照样可命中）。② **「上帧序」的存储形态**：命中读 `GetPaintOrder()` 与**同一次收敛**的 `paintIndex/paintEnd`（新增内部下标读），孩子由子树区间切出、逆序取；上次收敛之后被回收再分配的槽由 `paintIndex` 反查对不上而跳过。于是「当帧新建节点当帧不可命中」「本帧改 z 序不改变本帧命中」是**存储事实**而不是纪律。③ **`local ⊗ slotMatrix` 的 M1 形态住在命中器**：`HitTester.BindSlot(node, slot)` + `Slots`（渲染流槽表的只读视图），有效局部 = `Compose(local, slotMatrix)`；**不进 NodeTable 列、不改 P6 的 world 派生**——槽的产品写者是 M2-06 的 ScrollPane（滚动位移只写槽矩阵），在它进驻前给树加一列 slotId 等于给一个没有写者的字段编 ABI。未装 `Slots` 却绑了槽 = 断言（槽语义不猜）。④ **clip 剪枝的唯一数据面是 `IHitClipSource`**，由 `Extract` 直接实现（`ClipEntryOf` 读的就是它自己的 `_clipOf`，`TryGetClipWindow` 解析继承链并给出条目所在槽帧）；命中把 stage 点换算进该槽帧再比 rect。**结构上就没有第二份裁剪账**——这是 14b-4 死字段裁决（worldVisual 的 clip 域是保留段恒零、取值宏已删）的正面兑现，行为侧另有「渲染剔除的叶命中也剔除」的同判用例。⑤ **帧号护栏的可执行形态 = 「同帧同点不重复命中」**：一帧里先 move 再 down 是常态，位置变了必须重命中，否则 down 会派发到 move 之前的目标；位置没变的第二次问询（`TouchTarget`）复用缓存并计入 `HitCacheHits`。⑥ **`InputPacket` 字段复用约定**（M1-08 的 POD 已冻结，本平面明文化其解释面）：Pointer* 包的 `Id` = 指针 id、`Device` = **键号**（0 左 / 1 右 / 2 中）；Wheel 包的 `DeltaX/DeltaY` 必须是非零整数 canonical 值，零值与亚单位在入口被拒并计数。⑦ **触点槽抬起后不回收**：回收会连 `LastRollOver` 一起丢，下一次移动就重发一遍 RollOver；槽荒时淘汰「最久没动过的非按下槽」，十槽全在按下期才是真用尽（有声丢包）。⑧ **`EventCtx` 逃逸的编译负例落在 `tools/PropGen.Tests`**（存字段 = CS8345、闭包捕获 = CS1628，对**真实 Core 程序集**编译；主 runner 是零 NuGet 工程判不了编译错），主 runner 侧钉住 `IsByRefLike` 与内建 id 的常量除零门 `UiEvents.BuiltinMaskFits`。⑨ **`Ch.DownLayer` 的首个产品写者不在本包（成文裁决）**：命中缓存是帧内的（帧号护栏），不需要失效通道；绘制序的层归 `Ch.Structure`；真正会写它的是「整棵子树翻层」——孤岛 visual 并入下行（M1-23）与滤镜 RT 域翻层（M2-12，fork `Container.SetChildrenLayer` 同源）。本包把该通道的**排水路径**从零覆盖变成有行为用例（`Mark(DownLayer)` → P6 下钻 → 落叶交回 `Ch.Color/CascadeDown`），并留一道「归属表里出现首个 DownLayer 写者即红」的登记门逼出后续覆盖。⑩ **M1 明码边界四条**：焦点只是一个可写属性（`SetFocus` 的 LCA diff / FocusIn/FocusOut / `(listId, dataKey)` 寻址归 M2-10）；RollOver/RollOut 走 LCA 差集**直发**（不冒泡，避免共同祖先反复收 out/over）；世界空间面板的 `PanelTable` 按相机射线缓存未进驻（M1 单面板 stage 坐标直入）；拖拽/DragDrop/手势识别器是 `CaptureTouch`/monitor 之上的库代码，本包只交付基元。
...
>
> **段内定长记录也落数据表；规范段序与「组件内相对」两条口径定死（M1-20b 实现期补充）。** M1-19 只把头/目录/NODE 列做成 `Abi.cs` 的数据表，冻结各段时冒出的第二批记录（COMP/CONT/LOCL/LEAF/SEGS/TREF/STRT 头与条目/CNST 头）在本包收口为 `Abi.FgbRecords`（`AbiRecord` = 名字 + 定长 + 字段表），生成物出 `Fgb{Name}{Field}Offset/Size` + `Fgb{Name}Size` + 首尾相接的编译期断言 + `Verify` 交叉校验——写侧（`FgbFreezer`）与读侧（M1-22 装载、读回对账用例）**共用同一批偏移常量，文件里不出现任何字面偏移**，机制 8 的「FGB 全部记录布局集中为纯数据文件」自此对全段成立。段 fourcc 表续两个（append-only）：`CONT`（内容条目表 = `contentRef` 的目标，也是降级阶梯「组件级降回运行时 Extract」的源）与 `LOCL`（localId ⇄ 编辑器 id）。**规范段序**（M1-19 留缝 ① 的兑现，容器只按 `AddSection` 调用序排，排序决定归编译器）：`STRT → TREF → COMP → NODE → CONT → LOCL → CNST → QUAD → SEGS → LEAF → CLIP`，读作「身份与索引先行 → 模板本体 → 求值图 → 渲染冻结」；段序进 blob 字节 ⇒ 进 golden，改段序 = 改产物 = code review 里看得见。**组件内相对**是全部区间账的统一口径：NODE 拓扑列相对化为组件内 1 基下标（哨兵 0 保留，逆变换 `abs = instanceBase + rel − 1`，M1-22 消费 Rebase 位），LEAF 的 quad/seg/run/clip 下标、CNST 的 `FanOut.Start` 与算子下标池同样是组件内相对，读侧加 COMP 上的对应 `*Start` 归位——模板一旦冻死绝对下标就绑住了包内位置，同一模板在别的包里、别的基址上就不能复用。**边界写明**：编译世界里组件根恒在槽 1，故 NODE 相对化在 M1 是**数值恒等**（`firstAbs == 1`），它按 `firstAbs` 参数化并有独立用例钉住；真语料随 M1-22 的实例化基址回填出现，在那之前「摘掉相对化调用」不可观测（杀变异存活项，与 M1-20a 的 `LateSlotAllocs` 跳线同类）。
>
> **FGB v2：PLAN/PTCH/DEPS 三段进驻，头补 `pkgId`，组件根改判为子树根（M1-22 实现期补充）。** 格式版本 bump 到 2，落三件事。① **新三段 + COMP 长两字段**：`PLAN`（`InstStep` 16B：`compIndex/parentStep/kind/hostLocalId/listItemCount`，后序扁平，COMP 从 v1 的保留区切出 `planStart/planCount`，记录 96B→104B）、`PTCH`（16B：`texRef/target/section/slot`）、`DEPS`（16B：`{pkgId, expectedSourceHash}`）。② **头补 `pkgId`**（u64，从 v1 的 16B 保留区切，尾部仍留 8B）——装载门 5「文件名以 id 定身份」需要一个头内对照物，靠 TREF 里的自引用去反推是把身份挂在别的表上。③ **规范段序扩到十四段**：`STRT → TREF → DEPS → COMP → NODE → PLAN → CONT → LOCL → CNST → QUAD → SEGS → LEAF → CLIP → PTCH`，读作「身份与索引先行 → 模板本体 → 求值图 → 渲染冻结 → **装载期补丁**」；PTCH 排末尾不是美学：它是唯一会被写的段。**DEPS 写条目不写期望哈希**：依赖 id 在描述符里，被引用包的 `sourceHash` 不在单包编译面内，故逐条写 `{pkgId, 0}`——`0 = 未知`，装载门 4 据此记 `unverified` 计数而**不判不符**（编造一个对照物比不写更糟）；头内 `combinedRefHash` 同理恒零。M1-20b 留下的「DEPS 不写」由此改判为「写条目、期望哈希留零」，多包编译面仍未进驻。**组件根改判**：M1-20a 让组件根就是无头世界的树根，M1-22 改成**树根的孩子**——形态上对齐运行期（实例永远是子树），效果上让模板区间从槽 2 起，于是 NODE 拓扑列的相对化在真语料上**不再是数值恒等**；M1-20b 记在案的「摘掉 `Relativize` 调用不可观测」自此销账（注入该变异，编译产物 golden 与全部装载用例一起红）。配套一条冻结期规范化：**模板根的 `parent/nextSib/prevSib` 冻成哨兵 0**——根挂在哪儿是实例化的事，不是模板的一部分；装载期的拓扑门据此要求「局部 0 的三列全 0」。NODE 的 `ownerInst` 一并改判为**实例身份列**：模板里恒 0，实例化按块回填成块首下标。
>
> **装载三操作落地：门 4/5 上线、降级二分可执行、PLAN 同步实例化、fuzz 转常设门（M1-22 实现期补充）。** `FgbPackage.TryLoad(byte[], in FgbLoadOptions)` 是三操作的唯一入口，失败面仍是二值（拒载 = `package == null`；装载可能带降级标记），**永不抛**。**①验证**在容器门（M1-19 的 1-3）之后补上整层**段内**门，全部结构性、全部拒载：必备段缺席（12）、段内计数与段长不自洽（13，STRT 池窗口 / CNST 四数组账 / 十四段记录宽整除）、COMP 九组区间越段或 nodeCount 前缀和对不上 NODE 行数（14，另含逐行 `localId == 行内序号`、拓扑列相对下标 ≤ nodeCount、contentRef 在界）、PLAN 违反后序性质或 compIndex/hostLocalId 越界（15）、PTCH 目标越界（16）、以及 **NODE 拓扑门（20）**：模板的父/首子/兄弟环三方自洽且自根 DFS 恰好触达全部节点。最后一条不是洁癖——memcpy 出来的链会被 P6 的 paintOrder 展开**直接走**，一个不闭合的兄弟环在那里是死循环而不是错画面，装载器是信任边界的最后一层，环路只能死在这里。**门 4/5** 兑现 M1-19 留缝 ①②：门 5（文件名 `名字_id[.sN][.bX].fgb`，**以 id 定身份不以名字**，id/sN/bX 与头内比）归**结构性**——改名/改档没重烘是部署错配，必须响亮；门 4（DEPS 逐项 + `combinedRefHash` **重算** + 档位与宿主全局设定）归**降级**——内容陈旧要的是「慢但正确」。**降级二分**因此是可执行的而不是文档口径：`FgbGateClass` 是判据本体，两条路径各有用例（结构性 ⇒ 无包可用；哈希级 ⇒ 包可用、能实例化、能发射实例）。`LoadReport` 补齐**逐门判决表**（门号/分档/pass·skip·reject·degrade/人读细节）、patch 数、耗时、以及确定性的 `Describe()`（无时间戳无路径，进 CI 日志也进用例断言）。**②视图**：十四段一律以 `(offset,length)` 记住，取用时切 span / `Cast`，blob 一生不复制。**③绑定**分两半：装载期只做便宜的一半（TREF 符号 → 宿主运行期纹理编号，PTCH 据此**就地**回填 CONT.texId 与 SEGS.tex[slot]）；贵的一半（CONT 解码、纹理像素）留给首用。就地回填是破坏性的——同一数组二次装载被门 3 响亮拒，这是刻意取舍并有用例钉住。边界写明：段键里还会出现**运行期自有**的纹理编号（字形图集），它们不来自本包、不产回填条目，但有声（每个不同编号一条 Info）。**PLAN 同步实例化**：一个块 = 一次段分配 + 21 次 `Array.Copy` + 5 列基址回填（`abs = base + rel − 1`，哨兵 0 保留）+ 一次 ownerInst 整块回填 + resolved 槽**按编译产物的布局写集批量分齐**（M1-16 留缝兑现，`LateSlotAllocs` 恒零有用例）+ 一次 `LayoutEngine.Arm`。顶层块与嵌套块都是各自模板的 slab 段（bin 键 = `最高位 | 域号<<16 | 组件下标`，最高位必须置——0 是单槽 bin），嵌套块挂在宿主 localId 上并**下传宿主框**（位置归零、尺寸取宿主 resolved 框）。**arm-not-mount**：实例化只装填，CONT 一条没解、纹理一张没装；`FgbContentSource` 在首次 `TryDescribe`（= P7 首次 extract）时才 Realize，寻址走 ownerInst → 块 → 实例，O(1)。首用时**模板身份链**（逐 local 的 `localId/typeId/LOCL.editorIdHash` 续 FNV，**刻意不含任何可编辑状态**——arm 的承诺是吸收中间编辑）对不上 ⇒ 该实例 `Inactive` + 计数 + 零发射：不画，不画错。**Pool 雏形**不缓存实例对象：`Return` = 标记即死（段进 P9 队列），`Rent` 走同一条 `Instantiate`，全部列重新 memcpy——「复用后有残留」在结构上不可能，这是本设计愿意为之付一次 `Array.Copy` 的性质。**等价性金样的第三条腿**同时上线（M1-20b 边界 ① 销账）：FGB 装载实例化出的树，与 `TreeBuilder` 的树 21 根列逐位同、P5 收敛后的 resolved 逐位同、Extract 的 `CanonicalStream` **逐字节同**——门的两端自此各有独立的树来源。三条明码边界：文本叶的样式未进 M1 冻结面故解成「不产渲染单元」+ 计数（`TextLeavesUnbound`），带文本叶的组件不参与 resolved/Extract 两腿；带嵌套步的组件不参与 Extract 腿（嵌套子树在编译面上不存在）；宿主框只在装填那一刻下传，之后再变不随动（那需要 `parentUsesSize` 的随动约束，归 M2）。**fuzz 转常设门**：2048 轮、seed 固定，语料从容器四类扩到**十一种段内记录**（COMP/PLAN/PTCH/CONT/LOCL/CNST/STRT/LEAF/SEGS/DEPS/NODE），段内变异一律补钉 selfHash（否则一切停在门 3，段内那十几道门永远拿不到语料）；断言是「要么干净拒载（`package == null` + 门号），要么正常装载且**全视图可走 + 可实例化**」——「装得上但一 memcpy 就越界」不算装载成功。
...
> **前半落地：`FgbCompiler.Shape` 出「已定形的树」（M1-20a 实现期补充）。** 编译切成两半、以 `ShapedPackage` 为接缝：**Shape（已可用）** = .fui → 逐组件一个**真无头世界**（NodeTable/UiKernel/LayoutEngine/TextSystem/ContentTable 全是运行期同款实例，属性一律走正常 setter——编译器不绕写门）→ relation 经 `ConstraintCompiler` 翻译（24+Size → EdgeFollow 翻译表有 golden）喂 `ConstraintGraphBuilder.Seal`（**拒环门就是 Seal，无第二套**；环 = FGM101 错误 + 编辑器 id 闭合环路径，该组件除名、余者照编）→ 真跑 `UiKernel.Tick` 收敛 P5 与文本度量——幂等门与布局差分神谕**全程开启**，未收敛 = FGM901、门红或迟到槽分配非零 = FGM902（`LayoutStats.LateSlotAllocs` 恒零自此升格为编译门：resolved 槽在 Arm/RegisterMeasured 布防期全部预解，M1-16 留缝兑现）。**Freeze（M1-20b 进驻点）**吃 `ShapedPackage` 做 Extract/canonical/段冻结，不重新解析。诊断是 `FGM` 码词表（append-only，同 FgbGate 纪律）。
>
> **GGroup 真节点化的落地口径与三条明码边界（M1-20a 实现期补充）。** 组员按 `groupId` 重挂为组节点真孩子、坐标换入组空间（嵌套组逐级）；组条目在显示列表上位于其员之后（编辑器约定），真节点化后组块整体占组条目的绘制位——组员连续时与 fork 逐员绘制序逐位同，不连续出 FGM201（警告，归属照挂）。边界：① **跨组百分比位置关系不编**（FGM202）——FollowPercent 的锚是 src 起点绝对值，跨坐标系直写是错切；差值关系的常量系差在捕获时被吸收、百分比尺寸空间不变，两者照编。② **组员与组同轴双随动**（组员对组外目标有位置关系 ∧ 所在组同轴也被约束）在 fork 里是事件序竞态、这里是两次独立求值叠加——已知语义差异，登记待 M2-14 像素矩阵按 oracle 已知错豁免处理。③ 组归属非法（指向非组/越界/组链成环）按无组处理 + FGM203。
>
> **内容挂载的 M1 面（M1-20a 实现期补充）。** 进编译面的：整图/九宫图片叶（UV 按 sprite 矩形 ÷ 图集条目尺寸归一烘死；平铺位原样进 `SpriteRegion`，拒发已在 Extract 有声）、无线框直角矩形 Graph（纯色叶）、纯文本（块 5/6 按 fork `GTextField.Setup_BeforeAdd` 字段序解释成 `TextStyle`，autoSize 经 `RegisterMeasured` 进 P5 度量层——编译期度量 = `TextCore.Layout` 纯函数，与运行时逐位同）。有声不编（FGM303/302 计数，节点保几何）：裁白 trim、旋转入图集、MovieClip/Swf、线框与圆角 Graph、Loader（装载期动态物）、UBB 富文本（按纯文本排）、`autoSize=Shrink`（按不自适应排）、四类段键（Normal/Add/Multiply/Screen）之外的 BlendMode（回退 Normal）、双值 skew、跨包引用（归 PTCH/DEPS）。**清单即承诺**：不在表上的形态不许静默回退——回退而无声的编译产物会画错且没有任何信号，正是机制 4 存在的理由。字体是机制 13 font-map 的 M1 形态：`CompileOptions.Fonts` 注册（首条为回退），缺映射 = FGM301 警告 + 回退或跳过，**不是运行时缺字也不是崩溃**。
>
> **后半落地：`Freeze` 出 FGB，三道门同时上线（M1-20b 实现期补充）。** `FgbCompiler.Freeze(ShapedPackage)` 是唯一入口且**不重新解析 .fui**——前端在这一半根本不在场，产物只来自 20a 那些真无头世界。流程：逐组件跑**运行期同一个 `Extract`**（离线驱动 = 直调 `Rebuild()` + 自开 `PruneAfterRebuild` 补上管线 `DrainTail` 的那道尾剪枝）→ CONT/STRT/TREF 走 `CanonicalTable` 去重（机制 10：字节等同必同 id，id = 首次登记序，判等一律逐字节，哈希只找候选）→ 十一段按规范段序冻结 → 打内存计划。**冻结前置门 FGM903**：离线 Extract 读的是 P6 的产物（paintOrder + world/worldVisual），相位机里 P7 在 P6 之后所以自然满足，离线路径没有相位机替它排序——失败形态是「沿陈旧序按陈旧矩阵发射」，产物看起来完全正常。故冻结前自验 `DerivedMatchesFullRecompute` 与 `ValidatePaintOrder`，不成立即编译错（「上游 Tick 过所以一定新鲜」是关于实现的推断，不变量是关于产物的断言，两者必须各验一次）。canonical 的后置扫描（不变量 8）同理：按构造不可能红，仍然扫。
>
> **等价性金样比的是「两个驱动」，不是「两份实现」（M1-20b 实现期补充）。** 不变量 18 的落地形态：同一 .fui，路径 A = 编译器的离线驱动，路径 B = **真运行时管线**（`RenderPipeline.Attach` → 标结构脏 → `UiKernel.Tick` 走完 P6 定形 / P7 五通道排水 + DrainTail 剪枝 / P8 提交），两条流经 `CanonicalStream` 规范化（`structEpoch` 归零，同 `IncrementalGate` 的纪律）后**逐字节**比。两端跑的是同一个 `Extract` 算法——门证的是「离线捷径与管线全程等价」，这既是「编译器 = 无头运行时」的机器可验形式，也是降级阶梯「组件级降回运行时 Extract」照样画对的凭据。三条边界：① M1 的路径 B 复用 20a 建的那棵树（建树本身是运行时 API 的产物，20a 的「手工建等价树逐位同」用例守着），「另一份宿主从 FGB 装载后再 Extract」归 M1-22，那时门的两端才各有独立的树来源；② 六个样例包的编译产物**一个裁剪域都没有**（`overflow=scroll` 的容器裁剪归 M2-06），尾剪枝在真语料上恒剪 0，故金样另配一条**注入裁剪域**的用例把那条路径变成可观测的；③ 不变量 17（离线产物 == 进程内 JIT 产物）在单进程编译器里两端同一段代码，其可验形态随 M1-25 的预览宿主进驻。
>
> **内存计划是账本，也是断言锚（M1-20b 实现期补充，机制 11 落地）。** 每次编译打印确定性文本：`blob=NB sections=11` 头行 → 逐段字节（带 `records=N×WB` 或 `nodes/cols/bytesPerNode`）→ 逐组件（nodes/quads/segs/leaves/clips/ops/slots/instanceBytes/pool）→ `pool-budget`（= 实例化一份全部组件的定长开销：NODE memcpy + 实例块）→ `canonical`（去重后条数/登记次数，三张表各一列）→ `mask`（可观测属性掩码占用率 + 逐 PropId 分组占用）。文本无时间戳/路径/机器名、浮点走 `InvariantCulture`，于是它**同时**是：CI 日志里「第 65 个属性 / 实例块膨胀」的告警位置、code review 里 blob diff 的人读面（blob 只说「变了」，账单说「哪个段哪个组件变了」）、以及用例的断言锚（逐段账 == blob 实际段字节、逐行 pool == nodeCount×80B + instanceBytes、去重率分子分母）。M1 的边界写明：BIND 未进编译面故掩码行恒 `0/64`，报的真预算是**属性 id 空间**的分组占用；`instanceBytes` 恒 = 实例块头（控制器状态与定长 scratch 随 M2 状态层进驻）。伴生的 `ReactiveGraph` 文本（ABI 偏移复述 + CNST 逐组件拓扑序按编辑器 id 打印 + BIND 占位）与账单一起入 golden。
